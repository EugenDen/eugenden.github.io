...
Токен получается для IP. В примере - `requestip` - обозначает, что обращение к контенту будет осуществляться с текущей машины. Если вы делаете скрипт геообработки учитывайте, что окружение на машине создания скрипта и машине ArcGIS Server может различаться.
